...
class GeneExpressionDataset(Dataset):
    """
    PyTorch Dataset for gene expression and clinical survival data.

    This dataset handles:
    - Mapping patient case IDs to unique indices.
    - Extracting gene expression features.
    - Extracting binary survival labels (e.g., vital_status: Alive=1, Dead=0).
    - Including auxiliary survival time information (days_to_death).

    Args:
        data (pd.DataFrame): DataFrame containing columns:
            - 'case_id': unique patient identifier
            - 'days_to_death': survival time, may contain NaN for censored patients
            - 'vital_status': binary outcome label
            - other columns: gene expression or other numerical features
        case_id_to_idx (dict): Mapping from 'case_id' to a unique integer index,
            used for patient-level graph operations or batching.

    Returns:
        Each __getitem__ returns:
            - features (Tensor): gene expression features
            - label (Tensor): binary label
            - patient_idx (int): mapped patient index
            - days_to_death (Tensor): survival time or zero if censored

    Notes:
        - NaN values in 'days_to_death' are imputed as 0.0 to indicate right-censoring.
        - All numerical features except the label are cast to float32.
        - Duplicated rows in the DataFrame are dropped for safety.
    """
...
def filter_merged_data(merged_data, key_genes_file):
    """
    ...
    selected_gene_ids = bortua_genes | key_genes
...
def predict_on_test_set(
    ...
    """
    Run inference on a test set for a graph-based survival model,
    producing both classification (alive/dead) and optional survival time estimates.

    This function:
    - Runs the model in evaluation mode with no gradients.
    - Splits input features into gene features and clinical features.
    - Filters patient graph edges dynamically to match the current batch.
    - Computes binary predictions (alive=1, dead=0) based on sigmoid thresholding.
    - Optionally uses an external regressor to estimate days_to_death for predicted dead patients.
    - Logs basic classification performance (accuracy, AUC) and, if applicable,
      survival prediction error metrics (MAE, RMSE) on uncensored dead cases.

    Args:
        model (torch.nn.Module): Trained model with a forward method that returns
            both logits and risk scores.
        test_loader (DataLoader): PyTorch DataLoader yielding batches of:
            (features, labels, patient IDs, true days_to_death).
        edge_index_genes (torch.Tensor): Edge index tensor for the gene graph.
        edge_index_patients (torch.Tensor): Edge index tensor for the patient graph.
        num_clinical (int): Number of clinical features to slice from the feature matrix.
        regressor (sklearn-like model, optional): Trained regression model that maps
            risk scores to log-days-to-death. If None, survival time is not estimated.
        device (str): Device for inference (default: \"cuda\").

    Returns:
        pd.DataFrame: DataFrame containing for each sample:
            - case_id: patient identifier
            - predicted_label: binary prediction (1=alive, 0=dead)
            - probability_dead: probability output from sigmoid
            - true_label: ground truth binary label
            - risk_score: model’s risk estimate
            - true_days_to_death: ground truth survival time (may be 0 for censored)
            - estimated_days_to_death: regressor estimate or NaN
    """
    model.eval()
    model.to(device)
...
def main():
    """
    Main training pipeline for the GeneExpressionModel.

    This function orchestrates:
    - Argument parsing and output directory setup.
    - Logging configuration.
    - Input data loading, caching, and sampling to balance alive/dead cases.
    - Graph construction for genes (Pearson correlation) and patients.
    - Merging and preprocessing of gene expression and clinical data.
    - Dataset and DataLoader creation.
    - Model instantiation and training with checkpointing.
    - Model checkpoint averaging for final smoothing.
    - Survival regressor fitting on the learned risk scores.
    - Final test set prediction, including survival time estimates.
    - Extraction and saving of gene-level attention scores.
    - Export of top survival-associated genes for alive and dead patients.
    """
    ...
    merged_data_filtered = filter_merged_data(merged_data_sampled, key_genes_file)
