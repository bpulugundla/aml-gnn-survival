...
def read_and_process_data(gene_exp, gene_map, clinical_data):
    ...
    # Filter for relevant gene types
    # gene_mapping = gene_mapping[
    #    gene_mapping["gene_type"].isin(["protein_coding", "lncRNA", "miRNA"])
    # ]
    ...
    # Shuffle the dataset
    # final_data = final_data.sample(frac=1, random_state=None).reset_index(drop=True)
...
class GeneExpressionDataset(Dataset):
    def __init__(
        ...
        # Align centrality matrices by case_ids
        self.closeness_matrix = torch.tensor(
            closeness_matrix.loc[self.case_ids].values, dtype=torch.float32
        )
        self.eigenvector_matrix = torch.tensor(
            eigenvector_matrix.loc[self.case_ids].values, dtype=torch.float32
        )
        self.betweenness_matrix = torch.tensor(
            betweenness_matrix.loc[self.case_ids].values, dtype=torch.float32
        )
    ...
    def __getitem__(self, idx):
        ...
        closeness = self.closeness_matrix[idx]
        eigenvector = self.eigenvector_matrix[idx]
        betweenness = self.betweenness_matrix[idx]
...
def get_dataloaders(
    ...
    for phase in ["train", "valid", "test"]:
        ...
        # Extract all labels
        targets = np.array([label for _, label, _, _, _, _, _ in dataset])
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for (
            ...
            betweeness = betweeness.to(device)
            ...
            if dead_mask.sum() > 0:
                survival_loss = F.huber_loss(
                    risk_scores[dead_mask],
                    batch_days_to_death[dead_mask].log1p(),
                    delta=50.0,
                )
            else:
                survival_loss = 0.0
...
def build_centrality_matrix(
    case_model_dict, centrality_json_folder, centrality_key, key_genes=None
):
    """
    Constructs a centrality matrix with min-max normalized values for each gene per case.
    If a gene is in key_genes but missing in a model's centrality JSON, it is added with a score of 1.0.

    Args:
        case_model_dict (dict): Mapping from case_id to model_id.
        centrality_json_folder (str): Folder containing JSON files with centrality scores.
        centrality_key (str): The centrality score key to extract.
        key_genes (set or list, optional): Genes to force-include with a default score of 1.0.

    Returns:
        pd.DataFrame: case_id × gene centrality matrix (min-max scaled, float32).
    """
    key_genes = set(key_genes or [])
    all_genes = set()
    centrality_records = {}

    for case_id, model_id in case_model_dict.items():
        model_path = os.path.join(centrality_json_folder, f"{model_id}.json")
        ...
        gene_scores = {
            gene: info.get(centrality_key, 0.0) for gene, info in data.items()
        }

        if not gene_scores:
            continue

        genes, scores = zip(*gene_scores.items())
        scores_scaled = minmax_scale(scores)
        scaled_dict = dict(zip(genes, scores_scaled))

        # Insert key genes with value 1.0 if missing
        for gene in key_genes:
            if gene not in scaled_dict:
                scaled_dict[gene] = 1.0  # Default to neutral scaling

        all_genes.update(scaled_dict.keys())
        centrality_records[case_id] = scaled_dict

    sorted_genes = sorted(all_genes)
    result_df = pd.DataFrame(index=centrality_records.keys(), columns=sorted_genes)

    for case_id, gene_dict in centrality_records.items():
        for gene, score in gene_dict.items():
            result_df.at[case_id, gene] = score

    result_df = result_df.fillna(0.0).infer_objects(copy=False)
    return result_df.astype("float32")


def filter_merged_data(
    merged_data,
    json_folder,
    centrality_json_folder,
    case_model_tsv,
    key_genes_file,
    min_model_fraction=0.10,
):
    """
    ...
    Args:
        ...
        centrality_json_folder (str): Path to folder containing model JSON files with centrality scores.
        case_model_tsv (str): TSV file mapping case_id to model_id.
    ...
    gems_genes = get_gems_genes(json_folder, min_model_fraction)

    # Final set of selected genes
    selected_gene_ids = gems_genes | key_genes
    ...
    # Load mapping from case_id to model_id
    case_model_map = pd.read_csv(case_model_tsv, sep="\t")
    case_model_dict = dict(
        zip(case_model_map["case_id"], case_model_map["model_id"].astype(str))
    )

    closeness_matrix = build_centrality_matrix(
        case_model_dict,
        centrality_json_folder,
        centrality_key="closeness_centrality",
        key_genes=key_genes,
    )
    eigenvector_matrix = build_centrality_matrix(
        case_model_dict,
        centrality_json_folder,
        centrality_key="eigenvector_centrality",
        key_genes=key_genes,
    )
    betweenness_matrix = build_centrality_matrix(
        case_model_dict,
        centrality_json_folder,
        centrality_key="betweenness_centrality",
        key_genes=key_genes,
    )

    # Apply filtering to centrality matrices
    closeness_matrix = closeness_matrix[gene_columns]
    eigenvector_matrix = eigenvector_matrix[gene_columns]
    betweenness_matrix = betweenness_matrix[gene_columns]

    valid_case_ids = merged_data_filtered["case_id"]
    closeness_matrix = closeness_matrix.loc[
        closeness_matrix.index.intersection(valid_case_ids)
    ]
    eigenvector_matrix = eigenvector_matrix.loc[
        eigenvector_matrix.index.intersection(valid_case_ids)
    ]
    betweenness_matrix = betweenness_matrix.loc[
        betweenness_matrix.index.intersection(valid_case_ids)
    ]
...
def main():
    ...
    personalised_gems_jsons = "GEMS/gene_gene_via_metabolite_noiso_stats"
    centrality_jsons = "GEMS/gene_gene_via_metabolite_noiso_stats"
    case_model_map = "GEMs/case_model_mapping.tsv"
    gene_exp_file = (
    ...
    merged_data_filtered, closeness_matrix, eigenvector_matrix, betweenness_matrix = (
        filter_merged_data(
            merged_data_sampled,
            personalised_gems_jsons,
            centrality_jsons,
            case_model_map,
            key_genes_file,
        )
    ...
    alpha_value = trained_model.gnn.alpha.item()
    beta_value = trained_model.gnn.beta.item()
    gamma_value = trained_model.gnn.gamma.item()

    logging.info(f"Alpha (closeness weight): {alpha_value:.4f}")
    logging.info(f"Beta (eigenvector weight): {beta_value:.4f}")
    logging.info(f"Gamma (betweenness weight): {gamma_value:.4f}")
