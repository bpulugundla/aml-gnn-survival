...
class DualGraphGAT(nn.Module):
    def __init__(
        ...
        self.risk_head = nn.Linear((hidden_dim * 2) + num_clinical, 1)
    ...
    def forward(
        ...
        """
        x_genes: Gene expression matrix (batch_size, num_genes)
        top_k_indices: Top k gene indices
        edge_index_genes: Gene-gene graph
        edge_index_patients: Patient-patient graph
        clinical_features: ethnicity, age_at_diagnosis, race
        """
...
class GeneExpressionModel(nn.Module):
    def __init__(
        ...
        self.gnn = DualGraphGAT(
            batch_size, num_genes, num_clinical, hidden_dim, num_classes, dropout=0.3
        )
    ...
    def forward(
        ...
        """
        Args:
            x (torch.Tensor): Raw gene expression tensor of shape [batch_size, num_genes].
            edge_index_genes (torch.Tensor): Edge index tensor for the full gene-gene graph.
            filtered_edge_index_patients (torch.Tensor): Edge index tensor for the batch-specific patient-patient graph.
            clinical_features (torch.Tensor): Tensor of clinical covariates, shape [batch_size, num_clinical].

        Returns:
            output: Model output containing:
                - logits for classification
                - risk score for survival estimation
                (Exact structure depends on DualGraphGAT implementation.)
        """
